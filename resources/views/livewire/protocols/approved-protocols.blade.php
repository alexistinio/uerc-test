...
<div class="row {{ (str_contains(request()->path(), 'protocols')) ? '' : 'mb-4'}}" id="header_approved">
 ...
 <div class="d-flex align-items-end justify-content-between mb-2">
    ...
    @foreach($approvedProtocols as $protocol)
  ...
  <div class="py-2">
     ...
     <div class="row" style="<?php echo $protocol->type_of_review=='FR-Full Review' && Auth::user()->role=='CERC' ? 'opacity: 50%' : '' ?>">
     ...
     <div class="px-3 pt-3 pb-5" id="expandDiv{{ $protocol->id }}" 
        style="
            ...
            background-color: white; 
            display: none;
            <?php echo $protocol->type_of_review=='FR-Full Review' && Auth::user()->role=='CERC' ? 'opacity: 50%' : '' ?>">
          <div class="row">
         
            <div class="col">
            <div id="expandedActionButtons" class="mt-4 d-flex justify-content-between">
                    <div>
                    <button onclick="terminate(<?php echo json_encode($protocol->id) ?>)" class="btn btn-danger">Terminate</button>
                    ...
                    <div>
                        <div class="font-italic text-primary" style="cursor: pointer"
                            ...
                            data-content="
                            <?php 
                                if($protocol->first_decision && $protocol->first_decision_access==null){
                                    echo "<h6>This protocol is now Approved & On-going. Kindly wait for the Initial Certificate to be issued by UERC.</h6>";
                                }
                                if($protocol->first_decision_access && $protocol->progress_report==null || $protocol->first_decision_access && $protocol->final_manuscript==null){
                                  echo "<h6>Initial Certificate has now been issued. You may now attach the Progress Report and the Final Manuscript.</h6>";
                                }
                                if($protocol->progress_report && $protocol->final_manuscript){
                                  echo "<h6>This Protocol has now been sent to UERC for Final Approval/Completion.</h6>";
                                }
        ...
        <div class="row">
          ...
          <div class="row mt-5">
            ...
            <div class="card bg-light py-3" style="min-height: 187px">
                <div class="card-body">
                    ...
                    <div style="font-family: 'Roboto', sans-serif;">Initial Certificate of Approval:  
                        @if(Auth::user()->role=='Admin')
                        <button onclick='firstDecision_access(<?php echo json_encode($protocol->id) ?>,<?php echo json_encode($protocol->first_decision_access) ?>)' class="btn btn-<?php echo $protocol->first_decision_access==null ? 'primary' : 'dark' ?>"><?php echo $protocol->first_decision_access==null ? 'Grant Access' : date('M d, Y', strtotime($protocol->first_decision_access))  ?></button>
                        @endif
                    </div>
                    @if($protocol->first_decision_access != null)
                    <div class="font-italic text-primary" style="cursor: pointer"
          ...
          <div class="row mt-5 pt-4">
            <div class="col text-center">
                <button <?php echo ($protocol->progress_report && $protocol->final_manuscript && $protocol->ammendment_form && $protocol->final_report_form && $protocol->first_decision_access)  ? '' : 'disabled' ?> onclick="complete(<?php echo json_encode($protocol->id) ?>)" class="btn btn-success mx-2 px-3" style="height: 50px">Mark as Complete</button>
            </div>
          </div>
          @endif
