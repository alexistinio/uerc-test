...
<div class="row {{ (str_contains(request()->path(), 'protocols')) ? '' : 'mb-4'}}" id="header_approved">
 ...
 <div class="d-flex align-items-end justify-content-between mb-2">
    ...
    @foreach($terminatedProtocols as $protocol)
  ...
  <div class="py-2">
          ...
          <div class="d-flex">
            ...
            @endif
              ...
              <div class="col text-end py-2 d-none d-md-block">
                  ...
                  @if(Auth::user()->role == 'Admin')
                 <button disabled class="btn btn-outline-dark"><a href="/export_pdf/{{ $protocol->id }}" >Download <i class="fa-sharp fa-solid fa-download"></i></a></button>
                  @endif
     ...
     <div class="px-3 pt-3 pb-5" id="expandDiv{{ $protocol->id }}" 
            ...
            <div id="expandedActionButtons" class="mt-4 d-flex justify-content-between">
                    ...
                    <div>
                        <div class="font-italic text-primary" style="cursor: pointer"
                            ...
                            data-content="<h6>This protocol has been terminated. You may unterminate this protocol should you wish to return it to the process flow.</h6>" 
                            ><u>Protocol Status</u>
                        </div>
                    </div>
                    <div>
                        ...
                        @endif
                        <button onclick="edit_details(<?php echo json_encode($protocol->id) ?>)" class="btn btn-outline-secondary" data-bs-toggle="modal" data-bs-target="#edit_modal_1" disabled>Edit <i class="fa-sharp fa-solid fa-pen-to-square"></i></button>
                        <button onclick="delete_function(<?php echo json_encode($protocol->id) ?>,'<?php echo $protocol->title ?>')" class="btn btn-outline-danger">Delete <i class="fa-solid fa-trash"></i></button>
                    </div>
        ...
        <div class="row">
                ...
                <div class="card-body">
                    ...
                    @if($protocol->doc1==null)
                        <form class="ps-3" action="/u_doc1/{{ $protocol->id }}" enctype="multipart/form-data" id="u_doc1{{ $protocol->id }}" method="post">
                            @csrf
                            @method('PUT')
                            <input disabled class="form-control" onchange="load_doc1(<?php echo json_encode($protocol->id) ?>)" type="file" name="doc1" id="doc1">
                        </form>
                        @else
                        <div class="d-flex">
                           @if(Auth::user()->role=="Admin")
                    ...
                    <div class="d-flex justify-content-between align-items-center mt-2" style="border-bottom: 1px solid silver">
                      ...
                      <form action="/s_progress/{{ $protocol->id }}" enctype="multipart/form-data" id="s_progress{{ $protocol->id }}" method="post">
                        @csrf
                        @method('PUT')
                        <input disabled class="form-control" onchange="load_progress(<?php echo json_encode($protocol->id) ?>)" type="file" name="progress_report" id="progress_report">
                      </form>
                      @else
                      <div class="d-flex">
                ...
                <div class="card-body">
                    ...
                    <div class="d-flex justify-content-between mt-2" style="border-bottom: 1px solid silver">
                        ...
                        <form class="ps-3" action="/u_orreceipt/{{ $protocol->id }}" enctype="multipart/form-data" id="u_orreceipt{{ $protocol->id }}" method="post">
                            @csrf
                            @method('PUT')
                            <input disabled class="form-control" onchange="load_or(<?php echo json_encode($protocol->id) ?>)" type="file" name="or_receipt" id="or_receipt">
                        </form>
                        @else
                        <div class="d-flex">
                            @if(Auth::user()->role=="Admin")
                    ...
                    <div class="d-flex justify-content-between align-items-center mt-2" style="border-bottom: 1px solid silver">
                      ...
                      <form action="/s_finalmanu/{{ $protocol->id }}" enctype="multipart/form-data" id="s_finalmanu{{ $protocol->id }}" method="post">
                        @csrf
                        @method('PUT')
                        <input disabled class="form-control" onchange="load_finalmanu(<?php echo json_encode($protocol->id) ?>)" type="file" name="final_manuscript" id="final_manuscript">
                      </form>
                      @else
                      <div class="d-flex">
                ...
                <div class="card-body">
                    ...
                    <div class="d-flex justify-content-between align-items-center mt-2" style="border-bottom: 1px solid silver">
                      ...
                      <form action="/s_revreport/{{ $protocol->id }}" enctype="multipart/form-data" id="s_revreport{{ $protocol->id }}" method="post">
                        @csrf
                        @method('PUT')
                        <input disabled class="form-control" onchange="load_report(<?php echo json_encode($protocol->id) ?>)" type="file" name="reviewers_report" id="reviewers_report">
                      </form>
                      @else
                      <div class="d-flex">
                      @if(Auth::user()->role=="Admin")
                    ...
                    @else
                      <form action="/s_attachments/{{ $protocol->id }}" enctype="multipart/form-data" id="s_attachments{{ $protocol->id }}" method="post">
                        @csrf
                        @method('PUT')
                        <input disabled class="form-control" onchange="load_attachments(<?php echo json_encode($protocol->id) ?>)" type="file" name="protocol_attachments[]" id="protocol_attachments" multiple>
                     </form>
                     @endif
                    </div>
                    ...
                    <div class="d-flex justify-content-between align-items-center mt-2" style="border-bottom: 1px solid silver">
                      ...
                      <form action="/s_ammendment/{{ $protocol->id }}" enctype="multipart/form-data" id="s_ammendment{{ $protocol->id }}" method="post">
                        @csrf
                        @method('PUT')
                        <input disabled class="form-control" onchange="load_ammendment(<?php echo json_encode($protocol->id) ?>)" type="file" name="ammendment_form" id="ammendment_form">
                      </form>
                      @else
                      <div class="d-flex">
                    ...
                    <div class="d-flex justify-content-between align-items-center mt-2" style="border-bottom: 1px solid silver">
                      ...
                      <form action="/s_finalreport/{{ $protocol->id }}" enctype="multipart/form-data" id="s_finalreport{{ $protocol->id }}" method="post">
                        @csrf
                        @method('PUT')
                        <input disabled class="form-control" onchange="load_finalreport(<?php echo json_encode($protocol->id) ?>)" type="file" name="final_report_form" id="final_report_form">
                      </form>
                      @else
                      <div class="d-flex">
            ...
            <div class="mt-5 ms-3" style="font-size: 16px">Note: {{ $protocol->terminate_note }}</div>
